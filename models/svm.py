"""
Support Vector Machine (SVM) Regression Model for Agricultural Commodity Price Prediction.
"""
...
def run_svm(dataset_name: str = "babin", save_plots: bool = True, show_plots: bool = False):
    """Trains SVM Regressor model, calculates performance metrics, and plots visualizations."""
    if dataset_name.lower() == "all":
        results = {}
        for ds in config.TEAM_DATASETS:
            results[ds] = run_svm(dataset_name=ds, save_plots=save_plots, show_plots=show_plots)
        return results

    print(f"\n==========================================")
    print(f"🎯 Training Support Vector Regressor (SVR)")
    ...
    for member in ["babin", "liza", "ritika"]:
        if member in dataset_tag:
            dataset_tag = member
            break
    line_path = config.RESULTS_DIR / f"svm_line_{dataset_tag}.png" if save_plots else None
    bar_path = config.RESULTS_DIR / f"svm_bar_{dataset_tag}.png" if save_plots else None
    pie_path = config.RESULTS_DIR / f"svm_pie_{dataset_tag}.png" if save_plots else None

    # Visualizations
    plot_line_graph(avg_prices, model_name="SVM", save_path=line_path, show=show_plots)
    plot_bar_graph(avg_prices, model_name="SVM", save_path=bar_path, show=show_plots)
    plot_pie_chart(avg_prices, model_name="SVM", save_path=pie_path, show=show_plots)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Support Vector Regressor on Agricultural Price Data")
    ...
    run_svm(dataset_name=args.dataset, save_plots=args.save, show_plots=show_plots)
